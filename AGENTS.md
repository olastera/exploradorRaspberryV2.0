...
- En `views/music/album-grid.php` los archivos del listado ya vienen prefiltrados a solo audio (`if (!in_array($ext, $audioExtensions)) continue;`), así que un botón "Mou a Imatges" gateado por `$imageExtensions` ahí sería código muerto (nunca sería `true`) — deliberadamente no se añadió. Sí tiene sentido en `admin-movies.php` y `docs/file-list.php`, donde el listado no está prefiltrado por tipo.
